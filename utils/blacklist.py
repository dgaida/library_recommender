...
class Blacklist:
    ...
    def remove_from_blacklist(self, category, item):
        ...
        self.blacklists[category] = [
            bl for bl in self.blacklists[category]
            if not (
                bl["title"].lower().strip() == title_lower and
                (not author_lower or not bl.get("author") or 
                 bl.get("author", "").lower().strip() == author_lower)
            )
        ]
