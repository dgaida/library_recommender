...
def normalize_text(text):
    ...
    stop_words = {
        ...
        'before', 'after', 'above', 'below', 'between', 'among', 'der', 'die',
        'das', 'ein', 'eine', 'und', 'le', 'la', 'les', 'un', 'une', 'et'
    }
...
def fuzzy_match(search_terms, existing_folder, band, album) -> bool:
    ...
    band_found = any(
        any(band_variant in folder_version for folder_version in folder_versions)
        for band_variant in band_variants
    )

    # Album muss in einer der Ordner-Versionen gefunden werden
    album_found = any(
        any(album_variant in folder_version for folder_version in folder_versions)
