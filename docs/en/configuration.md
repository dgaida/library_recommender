...
- `data_sources/mp3_analysis.py`: Change the path in `add_top_artist_albums_to_collection("PATH/TO/YOUR/MP3/ARCHIVE", top_n=10)`.
...
By default, the app suggests 12 titles per category (4 per data source). This can be adjusted in `recommender/recommender.py`.
